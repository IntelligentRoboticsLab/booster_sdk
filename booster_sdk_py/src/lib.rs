...
use ::booster_sdk::{
    client::{
        AiClient, AsrConfig, BOOSTER_ROBOT_USER_ID, CameraControlStatus, CameraSetMode,
        DetectResults, GetStatusResponse as X5CameraStatusResponse, LightControlClient, LlmConfig,
        LuiClient, LuiTtsConfig, LuiTtsParameter, SpeakParameter, StartAiChatParameter, TtsConfig,
        VisionClient, X5CameraClient,
    },
    types::{
        Action, BodyControl, BoosterError, BoosterHandType, CustomModel, CustomModelParams,
        CustomTrainedTraj, DanceId, DexterousFingerParameter, Frame, GetModeResponse,
        GetRobotInfoResponse, GetStatusResponse, GripperControlMode, GripperMode,
        GripperMotionParameter, Hand, HandAction, JointOrder, LoadCustomTrainedTrajResponse,
        Orientation, Position, Posture, Quaternion, RobotMode, Transform, WholeBodyDanceId,
    },
};
use pyo3::{exceptions::PyException, prelude::*, types::PyModule};

use ::booster_sdk::client::{BoosterClient, GripperCommand};
...
impl PyAiClient {
    ...
    fn start_ai_chat(&self, py: Python<'_>, param: PyStartAiChatParameter) -> PyResult<()> {
        let client = Arc::clone(&self.client);
        let param: StartAiChatParameter = param.into();
    ...
    fn speak(&self, py: Python<'_>, param: PySpeakParameter) -> PyResult<()> {
        let client = Arc::clone(&self.client);
        let param: SpeakParameter = param.into();
...
impl PyLuiClient {
    ...
    fn start_tts(&self, py: Python<'_>, config: PyLuiTtsConfig) -> PyResult<()> {
        let client = Arc::clone(&self.client);
        let config: LuiTtsConfig = config.into();
    ...
    fn send_tts_text(&self, py: Python<'_>, param: PyLuiTtsParameter) -> PyResult<()> {
        let client = Arc::clone(&self.client);
        let param: LuiTtsParameter = param.into();
